...
public class RouteService {
    ...
    private static final List<String> FRANCHISE = List.of(
            ...
            "이마트에브리데이", "GS25", "CU", "세븐일레븐", "더벤티", "컴포즈", "블루보틀", "카페베네", "다이소", "롯데", "이마트"
    );
    ...
    public RouteResponse generate(GenerateRouteRequest req) {
        ...
        // 8) 응답
        List<String> names = chosen.stream().map(PlaceDto::getStore_name).toList();
    ...
    private RouteResponse toResponse(Route r) {
        return RouteResponse.builder().route_id(r.getId())
                .user_id(r.getUserId())
